...
describe("Flags", () => {
  ...
  describe("parses param flags", () => {
    ...
    it("fails on invalid param type", () => {
      ...
      expect(consoleErrorTrap).toHaveBeenLastCalledWith(
        expect.stringContaining("`--others` value is not an array")
      );
    });
    ...
    it("parses array params", () => {
      ...
      cli.run(
        undefined,
        withArgv("foo --name myname --others one --others two")
      );
      expect(fooAction.mock.lastCall?.[0].flags.others).toStrictEqual([
        "one",
        "two",
      ]);
    });
  ...
  test("flag char", () => {
    ...
    expect(() => {
      const barCmd = createCommand("foo")
