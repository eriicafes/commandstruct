...
describe("Run", () => {
    ...
    describe("error on unknown", () => {
        ...
        it("errors on unknown options", async () => {
            ...
            expect(consoleErrorTrap).toHaveBeenLastCalledWith(expect.stringContaining("Parsed unknown option"))
        ...
        it("errors on unknown options with callback error message", async () => {
            ...
            expect(consoleErrorTrap).toHaveBeenLastCalledWith(expect.stringContaining("bad flag -f"))
            expect(processExitTrap).toBeCalledTimes(1)
        })
    })
    ...
    describe("error handler", () => {
        ...
        it("rethrows command action error", () => {
            expect(run(prog, undefined, withArgv("foo"))).rejects.toThrow("something went wrong")
        })

        it("handles command action error with error handler", () => {
            expect(run(
                prog,
                {
                    onError(err) {
            ...
            expect(consoleErrorTrap).toBeCalledWith("handled error: something went wrong")
        })
    })
})
