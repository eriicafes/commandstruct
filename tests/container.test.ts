...
describe("Container", () => {
  ...
  const prog = createProgram("test", container)
    .commands(
      createCommand("foo").use<{ counter: Counter }>().action(fooAction),

      createCommand("bar").provide({ counter: Counter }).action(barAction),
  ...
  it("'use' reuses the parent container", async () => {
    ...
    expect(fooAction.mock.lastCall?.[1].counter).toBe(
      container.instances.counter
    );
  });
  ...
  it("'provides' creates a child container", async () => {
    ...
    expect(barAction.mock.lastCall?.[1].counter).not.toBe(
      container.instances.counter
    );
    expect(bazAction.mock.lastCall?.[1].counter).not.toBe(
      container.instances.counter
    );

    expect(barAction.mock.lastCall?.[1].counter).toStrictEqual(
      container.instances.counter
    );
    expect(bazAction.mock.lastCall?.[1].counter).toStrictEqual(
      container.instances.counter
    );

    container.instances.counter.count++;
    expect(barAction.mock.lastCall?.[1].counter).not.toStrictEqual(
      container.instances.counter
    );
    expect(bazAction.mock.lastCall?.[1].counter).not.toStrictEqual(
      container.instances.counter
    );
  });
});
