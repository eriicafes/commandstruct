  ...
  for (const [name, flag] of Object.entries(flags)) {
    ...
    if (flagObj.char !== undefined && flagObj.char.length !== 1) {
      throw new CommandError(
        "invalid_flag",
        `option ${
          "`" + Flag.toString(flag, name) + "`"
        } char must be 1 character`
      );
    }
    ...
    if (Flag.isNegated(key)) {
      ...
      if (existingKey && Flag.toObject(existingKey).param) {
        throw new CommandError(
          ...
          `negated option ${name} ${
            "`" + Flag.toString(flag, name) + "`"
          } can only be used with boolean flags`
        );
      }
    }
    program.option(
  ...
  for (const [name, arg] of Object.entries(args)) {
    ...
    if (argObj.type === "required" && hasOptional) {
      throw new CommandError(
        ...
        } cannot appear after an optional argument`
      );
    }
...
>(args: Args, flags: Flags, fnArgs: any[]) {
  ...
  for (const [name, flag] of Object.entries(flags)) {
    ...
    if (value === undefined) {
      ...
      throw new CommandError(
        "invalid_flag",
        `option ${"`" + Flag.toString(flag, name) + "`"} value is missing`
      );
    }
